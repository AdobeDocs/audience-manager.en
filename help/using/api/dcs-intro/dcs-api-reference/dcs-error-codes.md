...
Error codes and messages generated by the [!UICONTROL Data Collection Servers] ([!DNL DCS]) listed in numeric order by code ID.
...
|2|Invalid `d_orgid` value (could not find a config for this org id): `ID`|The Organization ID is incorrect. Check your ID and try the request again. If you do not know or have your Organization ID, see the "Administration Page" section [Organizations and account linking](https://experienceleague.adobe.com/docs/core-services/interface/manage-users-and-products/organizations.html) for information about how to find it.|
...
|111|Invalid `IMS` token received|Returned for [!DNL Audience Manager] - [!DNL Adobe Target] integrations. The error is thrown when a call is made to the [!DNL DCS], containing an invalid [!DNL IMS] token. The token might be malformed, expired or the user might not be authorized to access the required resource.|
...
The [!DNL DCS] returns error codes and messages in a [!DNL JSON] object or in an X- header in the HTTP response string.
